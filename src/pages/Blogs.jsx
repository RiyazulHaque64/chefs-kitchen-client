...
const Blogs = () => {
  ...
  return (
    <div>
      <div className="bg-img w-full h-48 lg:h-96 flex flex-col gap-2 items-center justify-center rounded">
        <h2 className="text-5xl text-white mb-4 lg:mb-10">Blog</h2>
        <div>
          <PDFDownloadLink document={<PdfFile />} fileName="blogs.pdf">
            {({ blob, url, loading, error }) => (loading ? <button className="bg-lime-200 rounded py-2 px-4 flex items-center gap-2 shadow-lg"><svg
            ...
            >
              <path
                ...
                fill="#6BAD0E"
              />
            </svg><span className="text-sm">Loading ...</span></button> : <button className="bg-lime-200 rounded py-2 px-4 flex items-center gap-2 shadow-lg"><BiDownload className="animate-bounce w-6 h-6 hover:animate-none" /><span className="text-sm">Download PDF</span></button>)}
          </PDFDownloadLink>
        </div>
      </div>
